...
export default function FloatingLabel(props: PropsFloatingLabel) {
  const { type, id, label, register, error, desc, placeholder, Icon } = props;
  return (
    <>
      <div className="relative">
        <Input
          ...
          {...register}
          className="peer h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 pr-14 text-sm font-medium text-slate-800 outline-none transition-all hover:border-slate-300 focus:border-emerald-500 focus:bg-slate-50 focus:ring-4 focus:ring-emerald-500/10 focus-visible:outline-none focus-visible:ring-0"
        />
        <label
          htmlFor={id}
          className="pointer-events-none absolute flex items-center gap-2 left-4 top-1/2 z-10 -translate-y-1/2 bg-slate-50 px-1 text-sm font-medium text-slate-400 transition-all duration-300 peer-focus:top-0 peer-focus:-translate-y-1/2 peer-focus:bg-slate-50 peer-focus:text-xs peer-focus:text-emerald-600 peer-[:not(:placeholder-shown)]:top-0 peer-[:not(:placeholder-shown)]:-translate-y-1/2 peer-[:not(:placeholder-shown)]:bg-slate-50 peer-[:not(:placeholder-shown)]:text-xs peer-[:not(:placeholder-shown)]:text-slate-500"
        >
