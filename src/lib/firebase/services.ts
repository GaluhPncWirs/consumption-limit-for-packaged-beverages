...
export async function pendingDeleteData(nameProduct: string) {
  try {
    const dataQuery = query(
      collection(firestore, "nutritionFact_pending"),
...
export function subscribeToFunFactSugars(
  callback: (funFactSugarEducation: educationsForFunfactSugar[]) => void
) {
...
export function subscribeToReleatedArtikel(
  callback: (sugarRelatedJournalsEducation: educationsForArtikel[]) => void
) {
...
export function subscribeToVideoEducation(
  callback: (videoEducationsEducation: educationsForVideo[]) => void
) {
